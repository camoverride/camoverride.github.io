...
Devanagari also contains many ligatures. Ligatures are created when two letters combine. For instance, when **ओ** (*au*) gets the chandrabindu diacritic, it becomes **🕉**, the *om* symbol sacred in Hinduism, Buddhism, Sihkhism, and Jainism. A ligature that Westerners might be most familiar with is the German **ß**, which occurs when two *s* letters combine, such as in the word **Straße** (*strasse*, street). Ligatures are not always easy to dissolve into their constituent sounds, and often need to be memorized.
...
Devanagari, an abugida, has numerous ligatures and diacritics that make it diffiult to create pangrams. However, Devanagari has an interesting organizational principle -- the magic square -- that can be used in language learning.

Chinese logograms are difficult to acquire and must be learned by hundrers of hours of rote memorization. 
